...
func getClientByName(c *gin.Context) {
    ...
    if err := c.BindJSON(&queryClient.NAME); err != nil {
        c.JSON(http.StatusBadRequest, gin.H{
            "status" : "Bad request",
        })
            fmt.Println(err)
    ...
    clientName := "%" + queryClient.NAME + "%"
    fmt.Println(clientName)

    // query for client info
    err := db.QueryRow(`SELECT * FROM Client WHERE name LIKE ?`, queryClient.NAME).Scan(&queryClient.ID, clientName, &queryClient.ADDRESS)
...
func getClientByNameQuery(c *gin.Context) {
    ...
    queryClient.NAME = string(clientName)

    fmt.Println(&queryClient.NAME)
